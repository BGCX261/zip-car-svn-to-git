...
function formataDataMMYYYY(campo, evt) {
    var xPos = PosicaoCursor(campo);
    //dd/MM/yyyy
    evt = getEvent(evt);
...
// limpa todos caracteres que não são números, menos o traço "-"
function filtraNumerosComTraco(campo) {
    var s = "";
    var cp = "";
    vr = campo;
    tam = vr.length;
...
function formataCEP(campo, evt) {
    ...
    if (!(tecla == "109" || // Se não for algum dos caracteres abaixo remove eles. (96 a 105 é de 0 a 9) e 109 é traço "-"
          tecla == "96" ||
          tecla == "97" ||
          tecla == "98" ||
          tecla == "99" ||
...
function formataPlaca(campo, evt) {
    ...
    tam = campo.value.length;
    // Se não for algum dos caracteres abaixo remove eles. (65 a 90 é de a a z) e 109 é traço "-"
//    if (tam <= 3 && (tecla >= 96 && tecla <= 105)) {
//        mascara(campo, soNumeros);
//        return;
//    }

    if ( !(tecla = "109" || (tecla >= "65" && tecla <= "90") || (tam != 4 && tecla == "109"))) 
    {
        mascara(campo, soNumeros);
        return;
    }
...
function mascara(o, f) {
    v_obj = o
    v_fun = f
    setTimeout("v_obj.value=v_fun(v_obj.value);", 0)
}
...
function abrirPopUp(URL) {
  ...
  window.open(URL,'Garagens', 'width='+width+', height='+height+', top='+top+', left='+left+', scrollbars=yes, status=no, toolbar=no, location=no, directories=no, menubar=no, resizable=no, fullscreen=no');
